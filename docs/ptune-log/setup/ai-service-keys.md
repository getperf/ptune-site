...
3. LLM設定に移動して以下の項目を入力します：
    ...
    - **モデル (model)**  
      利用するモデル名を選択
    - **Embeddingモデル**  
      タグ類似度検索用のベクトルDB検索で使用するモデル。Embedding モデルを設定すると、
      タグ類似度検索のたびに API にアクセスし、　**Embedding API の利用料が課金されます**。使用しない場合は「未使用」を選択してください。  
      Embedding の課金は一般的に非常に低く、
      数百検索を行っても月 1ドル程度に収まるケースがほとんどです。
...
- `ptune-log` では以下のテンプレートを標準で用意しています：
